...
export function SkillGraph({ exam, states, recall, misconceptions, highlight, selected, onSelect, details, onClose }: Props) {
  ...
  useEffect(() => {
    const load = (o: Orientation) =>
      Object.fromEntries(Object.entries(readStore<Record<string, Point>>(keyFor(NODES_KEY, o, exam)) ?? {}).filter(([id]) => id in BASE[exam][o].pos));
  ...
  useEffect(() => {
    ...
    (["horizontal", "vertical"] as const).forEach((o) =>
      writeStore(keyFor(NODES_KEY, o, exam), Object.keys(moved[o]).length ? moved[o] : null)
    );
  }, [loaded, moved, exam]);
