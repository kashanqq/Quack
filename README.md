...
- **Известные ограничения MVP:** если закрыть вкладку во время ответа ассистента, чат занят до `CHAT_LOCK_TTL_S` (2 мин); серверный Quack! фазы 4 пока не подключён к фронту (Quack! считается в браузере); Google-вход отключён.
