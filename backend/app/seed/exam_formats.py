...
def _as_json(value: dict) -> str:
    """Neo4j properties can't hold maps — store dict fields as JSON text.

    canonical.get_exam_format parses them back with json.loads.
    """
    ...
    async with driver.session() as session:
        ...
        for sec in data.sections:
            await session.run(
                ...
                n_items=sec.n_items,
                minutes=sec.minutes,
                item_types=_as_json(sec.item_types),
                scoring_rule=sec.scoring_rule,
                calculator=sec.calculator,
                adaptive=sec.adaptive,
                area_shares=_as_json(sec.area_shares),
                difficulty_shares=_as_json(sec.difficulty_shares),
        ...
        if data.scale_table:
            await session.run(
                ...
                """,
                exam_id=data.exam_id,
                raw_to_scaled=_as_json(data.scale_table),
            )
            report.nodes += 1
            report.rels += 1
